...
public class PutRequestSteps {
	...
	public void a_user_should_exist_with_user_id_name_and_job_in_the_system(String userId, String name, String job) {
	    ...
	    response=request.pathParam("userId", userId).contentType(ContentType.JSON).body(user).when().put("/users/{userId}");
	}
	...
	public void updated_user_reponse_should_contains_name_and_job_values(String name, String job) {
		json.assertThat()
